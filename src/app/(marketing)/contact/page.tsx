...
// Theme: Dark Cyber-Industrial (Comprehensive Corporate Dossier with Functional Links)
...
export default function AboutPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0A0B0E]/85 text-[#F3F4F6] pt-32 pb-24 px-6 relative z-10 flex flex-col items-center">
      ...
      {/* FULLY FILLED BOTTOM UTILITY & NETWORK GRID BAR */}
      <div className="w-full max-w-7xl mx-auto bg-[#0E1116] border border-[#262B36] rounded-2xl p-8 grid grid-cols-1 md:grid-cols-3 gap-6 items-center relative z-10 shadow-2xl mb-12">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-[#14171D] border border-[#262B36] flex items-center justify-center text-xl text-[#FFC700]">
            🌐
          </div>
          <div>
            <p className="text-[10px] text-gray-500 font-mono uppercase tracking-widest">Global Network Node</p>
            <p className="text-sm font-bold text-white font-mono">Coimbatore Industrial Grid, TN</p>
          </div>
        </div>

        {/* Functioning Social Media Placeholders */}
        <div className="flex justify-center gap-3">
          <button onClick={() => alert("Redirecting to Facebook profile...")} className="bg-[#3b5998] text-white w-10 h-10 flex items-center justify-center rounded-xl font-bold text-sm shadow cursor-pointer hover:opacity-80 transition-opacity">f</button>
          <button onClick={() => alert("Redirecting to X (Twitter) profile...")} className="bg-black border border-gray-800 text-white w-10 h-10 flex items-center justify-center rounded-xl font-bold text-sm shadow cursor-pointer hover:opacity-80 transition-opacity">𝕏</button>
          <button onClick={() => alert("Redirecting to LinkedIn profile...")} className="bg-[#0077b5] text-white w-10 h-10 flex items-center justify-center rounded-xl font-bold text-sm shadow cursor-pointer hover:opacity-80 transition-opacity">in</button>
        </div>

        {/* Functioning Email and SMS Triggers */}
        <div className="flex justify-end gap-3">
          <a href="mailto:info@gecogrinding.com" className="bg-[#14171D] hover:bg-[#262B36] border border-[#262B36] text-white text-xs font-bold uppercase tracking-wider px-5 py-3.5 rounded-xl transition-colors text-center">
            ✉ Send E-mail
          </a>
          <a href="sms:+9108047652002" className="bg-[#14171D] hover:bg-[#262B36] border border-[#262B36] text-white text-xs font-bold uppercase tracking-wider px-5 py-3.5 rounded-xl transition-colors text-center">
            📱 Send SMS
          </a>
        </div>
      </div>

      {/* COMPREHENSIVE CYBER-INDUSTRIAL FOOTER */}
      <footer className="w-full max-w-7xl mx-auto bg-[#050507] border border-[#262B36] rounded-2xl p-10 relative z-10 shadow-2xl">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-10">
          
          <div className="space-y-3">
            <h3 className="text-xl font-black uppercase text-white tracking-wider">GECO</h3>
            <p className="text-[#FFC700] text-xs font-bold tracking-widest uppercase">Grinding Centre</p>
            <p className="text-gray-500 text-xs font-mono leading-relaxed">Heavy-duty crushing machinery and precision engineering solutions built for extreme industrial operations.</p>
          </div>

          <div className="space-y-3">
            <h4 className="text-xs font-bold text-white uppercase tracking-widest border-b border-[#262B36] pb-2">Directory</h4>
            <ul className="space-y-2 text-xs text-gray-400 font-mono">
              ...
              <li><Link href="/products" className="hover:text-[#FFC700] transition-colors">/ Product Catalog</Link></li>
              <li><Link href="/about" className="hover:text-[#FFC700] transition-colors">/ About Us</Link></li>
              <li><Link href="/contact" className="hover:text-[#FFC700] transition-colors">/ Contact Us</Link></li>
            </ul>
          </div>

          <div className="space-y-3">
            <h4 className="text-xs font-bold text-white uppercase tracking-widest border-b border-[#262B36] pb-2">Infrastructure</h4>
            <ul className="space-y-2 text-xs text-gray-400 font-mono">
              <li>40,000+ Sq Ft Factory Unit</li>
              <li>2.5 Acres Logistics Yard</li>
              <li>Advanced Quality & Testing</li>
              <li>Certified IndiaMART Partner</li>
            </ul>
          </div>

          <div className="space-y-3">
            <h4 className="text-xs font-bold text-white uppercase tracking-widest border-b border-[#262B36] pb-2">Direct Inquiry</h4>
            <p className="text-xs text-gray-400 font-mono">Support Helpline:</p>
            {/* Functioning Phone Dialer Link */}
            <a href="tel:+9108047652002" className="text-xl font-bold text-[#FFC700] font-mono hover:text-white transition-colors block">
              08047652002
            </a>
            <p className="text-[10px] text-gray-500 font-mono">Singanallur, Coimbatore - 641005</p>
          </div>

        </div>

        <div className="border-t border-[#14171D] pt-6 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-gray-500 font-mono">
          <p>© 2026 Geco Grinding Centre. All Rights Reserved.</p>
          <p className="text-[#FFC700]">Verified Partnership Firm // GST: 33AABFG1064F2ZZ</p>
        </div>
      </footer>
    </main>
  );
